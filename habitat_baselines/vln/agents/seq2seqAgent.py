...
class seq2seqAgent(habitat.Agent):
    ...
    def train_step(self, n_iter):
        if (
            ...
            print("The resulting loss is ", self.loss.item() / self.episode_len / (n_iter % 10) )
            writer.add_scalar('Loss/train', self.loss.item() / self.episode_len / (n_iter % 10)  , n_iter)
            if self.loss:
                self.losses.append(self.loss.item() / self.episode_len / (n_iter % 10) )
            self.loss = 0
